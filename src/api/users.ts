...
export async function getUserById(id: number, viewerId?: number): Promise<User> {
...
export async function uploadProfilePicture(id: number, file: File): Promise<User> {
...
export async function deleteProfilePicture(id: number): Promise<void> {
  await requestEmpty(`${API_URL}/users/${id}/profile-picture`, {
    method: 'DELETE',
  });
}
...
export async function followUser(targetUserId: number): Promise<void> {
  await requestEmpty(`${API_URL}/users/${targetUserId}/follow`, {
    method: 'POST',
  });
}
